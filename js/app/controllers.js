...
angular.module('app.controllers', [])
	.controller('AppCtrl', function ($scope, $http) {
		...
		$http.get('https://mcapi.ca/query/play.hexagonminecraft.com/info').then(function successCallback(response) {
		...
		var momentStart = moment(serverData.startDate, 'YYYYMMDD').fromNow();
		$scope.startDate = momentStart.replace(/\D/g, '');
		$scope.memberCount = serverData.members.length;
		$scope.members = {};

		var playApiBase = 'https://api.minetools.eu/uuid/';
		angular.forEach(serverData.members, function (value, key) {
			var uuid = value.replace(/-/gi, '');
			$http.get(playApiBase + uuid).then(function (res) {
